...
class DrainTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_both_modes_preserve_incoming_and_outgoing_frame_limits(self):
        for mode in ("collect", "drain"):
            for direction in ("receive", "send"):
                with self.subTest(mode=mode, direction=direction):
                    bounded, wire = self.make_client([event(params={"text": "x" * 1000})],
                        notification_mode=mode, max_frame_bytes=256, max_total_bytes=512)
                    params = {"text": "x" * 1000} if direction == "send" else {}
                    with self.assertRaisesRegex(client.BoundedProtocolError, "frame limit"):
                        await bounded.request("thread/read", params, request_id=1)
                    self.assertTrue(wire.closed)
                    self.assertEqual(len(wire.sent), 0 if direction == "send" else 1)

    async def test_collect_preserves_connection_wide_bidirectional_byte_limit(self):
        ...
        bounded, wire = self.make_client(
            ...
            notification_mode="collect", max_frame_bytes=128, max_total_bytes=200,
        )
        await bounded.request("thread/read", {}, request_id=1)
    ...
    async def test_drain_accepts_more_than_24mb_one_rpc_and_across_rpcs_without_retention(self):
        for batches in ((400,), (200, 200)):
            with self.subTest(batches=batches):
                previous = None
                validated = 0
                ...
                def frames():
                    for request_id, size in enumerate(batches, 1):
                        for _ in range(size):
                            yield event(params={"text": "x" * 65536})
                        yield {"id": request_id, "result": {"exact": request_id}}

                bounded, wire = self.make_client(frames(), notification_validator=validate,
                    max_frame_bytes=131072, max_total_bytes=24_000_000)
                original = bounded._handle_bounded_notification
                ...
                with mock.patch.object(client, "write_ndjson") as trace:
                    for request_id in range(1, len(batches) + 1):
                        result = await bounded.request("thread/read", {}, request_id=request_id)
                        self.assertEqual(result.result, {"exact": request_id})
                        self.assertEqual(result.notifications, ())
                        self.assertIsNone(previous())
                    trace.assert_not_called()
                self.assertGreater(bounded._core.total_bytes, 24_000_000)
                self.assertEqual(bounded.profile.max_total_bytes, 24_000_000)
                self.assertIsNone(bounded._core.max_total_bytes)
                self.assertEqual(bounded._core.max_frame_bytes, 131072)
                self.assertEqual(validated, 400)
                self.assertEqual(bounded._notification_count, 0)
                self.assertIsNone(bounded._core.pending_messages)
                self.assertEqual(len(wire.sent), len(batches))
                self.assertFalse(wire.closed)

    async def test_drain_also_omits_lifetime_quota_for_sent_requests(self):
        bounded, wire = self.make_client(
            [{"id": request_id, "result": {"ok": True}} for request_id in range(1, 5)],
            max_frame_bytes=256, max_total_bytes=256,
        )
        for request_id in range(1, 5):
            result = await bounded.request("thread/read", {"text": "x" * 100}, request_id=request_id)
            self.assertEqual(result.result, {"ok": True})
            self.assertEqual(result.notifications, ())
        sent_bytes = sum(len(json.dumps(request, separators=(",", ":")).encode()) for request in wire.sent)
        self.assertGreater(sent_bytes, 256)
        self.assertFalse(wire.closed)
    ...
    async def test_drain_keeps_main_pre_send_duplicate_reservation_in_both_rpc_error_modes(self):
        for preserve in (False, True):
            with self.subTest(preserve=preserve):
                bounded, wire = self.make_client(
                    [event(), {"id": 1, "result": {"first": True}},
                     {"id": 1, "result": {"must_not_read": True}}],
                    preserve_rpc_errors=preserve,
                )
                result = await bounded.request("thread/read", {}, request_id=1)
                self.assertEqual(result.result, {"first": True})
                self.assertEqual(result.notifications, ())
                received = wire.received
                with self.assertRaisesRegex(client.BoundedProtocolError, "duplicate"):
                    await bounded.request("thread/read", {}, request_id=1)
                self.assertEqual(len(wire.sent), 1)
                self.assertEqual(wire.received, received)
                self.assertTrue(wire.closed)
